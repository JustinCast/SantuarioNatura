...
export class CommentService {
  ...
  acceptComment(_comment: Comments) {
    this._http
      .put(`${environment.port}acceptComment`,_comment)
      .subscribe(
        () => {
  ...
  getComments(): void {
    this._http
      .get<any>(`${environment.port}getComments`)
      .subscribe(
  ...
  getCommentsToConfirm(): void {
    this._http
      .get<any>(`${environment.port}getCommentsToConfirm`)
      .subscribe(
        data => {this.commentsToConfirm=data} ,
        (err: HttpErrorResponse) => {
          this.handleError(err);
        }
      );
